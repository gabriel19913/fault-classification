...
import time
import numpy as np
from training_sktime_robson import open_folds, find_max_value, normalizing, print_results
...
def training_rf_new(signal_type):
    INPUT_DATA_PATH = '../input-data/'

    cycles = ['cycle_1', 'cycle_2', 'cycle_4', 'cycle_8', 'cycle_16', 'cycle_32', 'cycle_64',
              'cycle_128']
    model_name = 'random_forest'
    print(f'# Treinamento do modelo usando RandomForest dataset2 sinal: {signal_type}')
    for cycle in cycles:
        print('\n---')
        c = cycle.split('_')[-1]
        if c == '1':
