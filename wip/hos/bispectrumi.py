...
from hos.tools import *

np.set_printoptions(linewidth=120)

def bispectrumi(y, type, nlag=None, nsamp=None, overlap=None,
  ...
  parameters = {'xtick.labelsize': SMALL_SIZE,
                'ytick.labelsize': SMALL_SIZE}
  plt.rcParams.update(parameters)
  cont = plt.contourf(waxis, waxis, abs(Bspec), 100, cmap=plt.cm.Spectral_r)
  plt.colorbar(cont)
  plt.title('Idirect Bispectrum fault ' +  type, fontsize=BIGGER_SIZE)
  plt.xlabel('F1', fontsize=MEDIUM_SIZE)
  plt.ylabel('F2', fontsize=MEDIUM_SIZE)

  plt.savefig('fig/' + type + '.png', bbox_inches='tight')
